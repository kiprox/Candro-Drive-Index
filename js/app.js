// Redesigned by t.me/KiPRoX from https://github.com/kiprox/Candro-Drive-Index which was written by someone else, credits are given on Source Page.
...
function title(path) {
	...
	// testing $('title').html(document.siteName + ' - ' + path);
	var model = window.MODEL;
	if (model.is_search_page)
		$('title').html(`${drive_name} - Search results for ${model.q} `);
	else
		$('title').html(document.siteName); //$('title').html(`${drive_name} - ${path}`);
}
...
function nav(path) {
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
	...
	html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Menu Navigasi</a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item"  href="/${cur}:/ ">> Home</a>`;
...
function markdown(el, data) {
	if (window.md == undefined) {
		$.getScript('https://cdnjs.cloudflare.com/ajax/libs/markdown-it/12.0.2/markdown-it.min.js',function(){
